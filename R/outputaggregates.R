...
OutputAggregates <- function( # Calculate and output aggregated rates and numbers of deaths at the country,
  ...
  regiontypes.select = c("UNICEF", "MDG", "WHO", "WB", "UNPD", "OIC",
                         "Countdown", "ECAAfrica", "AU",
                         "Fragile2013", "Fragile2014", "Fragile2015",
  ...
  year.target = 2015.5, ##<< MDG target year.
  ...
  if (!is.null(regiontypes.select)) {    #PROBABLY DELETE THIS ONE
    ...
    if (is.element("UNICEF", regiontypes.select))
      GetRegionalResults(regiontypes = UNICEFRegionAll,
...
CalculateCountryDeaths <- function(
  ...
  C <- nrow(u5mr.ct)
  nyears <- ncol(u5mr.ct)
  death0.ct <- death1to4.ct <- deathu5.ct <- matrix(NA, C, nyears)

  # calculate parameters
  q1to4.ct <- 1-(1-u5mr.ct/1000)/(1-imr.ct/1000)
  M0.ct <- (imr.ct/1000)/(1-(imr.ct/1000)*(1-a0.c))
  M0.ct[arr.ind.select] <- 0
  M1to4.ct <- q1to4.ct/(4*(1-q1to4.ct*(1-a1to4.c)))
  M1to4.ct[arr.ind.select] <- 0

  # calculate country deaths
  for (i in 1:nyears) {
    death0.ct[,i] <- M0.ct[,i]*pop0.ct[,i]
    death1to4.ct[,i] <- M1to4.ct[,i]*pop1to4.ct[,i]
    deathu5.ct[,i] <- death0.ct[,i] + death1to4.ct[,i]
  }
...
CalculateWorldDeaths <- function(
  ...
  for (j in 1:nsim) {
    for (i in 1:nyears) {
      ...
      q0.wtj[, i, j] <- M0.wtj[, i, j]/(1+(1-ifelse(mean(a0.c, na.rm = T) < 0.2, 0.1, 0.3))*
                                          M0.wtj[, i, j])
      q1to4.wtj[, i, j] <- 4*M1to4.wtj[, i, j]/(1+(4-4*mean(a1to4.c, na.rm = T))*M1to4.wtj[, i, j])
...
CalculateRegionalDeaths <- function(
  ...
  for (r in 1:nregs) {
    if (filename %in% c("UNICEFRegion", "MDGRegion", "WBRegion", "UNPDRegion", "OICRegion", "M49Region")) {
    ...
    for (i in 1:nyears) {
      ...
      q0.rt[r, i] <- M0.rt[r, i]/(1+(1-ifelse(mean(a0.c[select.reg], na.rm = T) < 0.2, 0.1, 0.3))*M0.rt[r, i])
      q1to4.rt[r, i] <- 4*M1to4.rt[r, i]/(1+(4-4*mean(a1to4.c[select.reg], na.rm = T))*M1to4.rt[r, i])
