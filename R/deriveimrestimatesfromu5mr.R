...
DeriveIMREstimatesFromU5MR <- function( # Derive IMR estimates from U5MR using model life table or Sahel equation
  ...
  # edit DJS 2017-06-07
  # is this a country with a special constant period (i.e. Yemen or Syria)
  special.constant <- !is.null(mcmc.meta$settings$special.constant.list)
  if(special.constant){
    cat("Deriving IMR for country with special constant period")
  }
  # end edit DJS
  ...
  # edit DJS 2017-06-07
  if(special.constant){
    res.cqt.Lw.u5.flat.med <- res.cqt.Lw[[which(names(res.cqt.Lw)==weight.alpha.publish)]][,2,]
      if(file.exists(file.path("output", runname.U5MR, "u5.ctj.flat.rda"))){
        load(file.path("output", runname.U5MR, "u5.ctj.flat.rda"))
        u5.ctj.flat <- u5.ctj.flat[1,,]
      } else {
        cat("Error: u5.ctj.flat.rda not found for country with special constant period")
      } # is file exists
  } # if special constant
  # end edit DJS
    ...
    if (!is.element(weights.alpha.plusdefault[w], weights.alpha.not.select)) {
      ...
      if (!is.element(mcmc.meta$data.all$imrmethod.c, c("B3", "Nonstandard"))) {
        ...
        if (mcmc.meta$data.all$crisisadj.c) {
          ...
          # edit DJS 2017-06-07
          if(special.constant){
            u.median.t.flat <- apply(u5.ctj.flat, 1, median)
            propadj.t <- GetCrisisAdjustedEstimates(u.t = u.median.t.flat, year.t = year.t, 
                                                    iso = mcmc.meta$data.all$iso.c, operation = "-",
                                                    adj.file = adj.file.U5MR)$propadj.t
            u5.crisisfree.tj.flat <- apply(u5.ctj.flat, 2, "*", propadj.t)
          }
          # end edit DJS
        } else {
          ...
          ## edit DJS 2017-06-07
          if(special.constant){
            u5.crisisfree.tj.flat <- u5.ctj.flat
          }
          ## end edit DJS
        }
        ...
        if (mcmc.meta$data.all$hiv.c) {
          ...
          ## edit DJS 2017-06-07
          if(special.constant){
            u5.hivcrisisfree.tj.flat <- u5.crisisfree.tj.flat
          }
          ## end edit DJS
        }
        # save for checking
      ...
      if (is.element(mcmc.meta$data.all$imrmethod.c, c("B3", "Nonstandard"))) {
        ...
        u1.hivcrisisfree.tj <- apply(u5.hivcrisisfree.tj, 2, GetEstimateViaMLT,  
                                     result = "IMR", 
                                     lifetable = sub("MLT ", "", mcmc.meta$data.all$imrmethod.c), lifetable.file = lifetable.file)
        # edit DJS 2017-06-07
        if(special.constant){
          u1.hivcrisisfree.tj.flat <- apply(u5.hivcrisisfree.tj.flat, 2, GetEstimateViaMLT,  
                                       result = "IMR", 
                                       lifetable = sub("MLT ", "", mcmc.meta$data.all$imrmethod.c), lifetable.file = lifetable.file)
        }
        # end DJS edit
        ...
        if (sum(!select.nonNA) > 0) {
          ...
          if (length(year.t[year.t >= 1985.5 & ntrajs.NA > 0]) > 0) {
            cat(paste0("Note: ", mcmc.meta$data.all$name.c, " ", mcmc.meta$data.all$iso.c, " - The years (only 1990.5 onwards shown) ", 
                       paste(year.t[year.t >= 1985.5 & ntrajs.NA > 0], collapse = ", "),
                       " have ",
                       paste(ntrajs.NA[year.t >= 1985.5 & ntrajs.NA > 0], collapse = ", "),
          ...
          # note: we need estimates from at least 1990! changed to 1985!
          year.cutoff <- ifelse(length(year.t[!select.nonNA & year.t < 1985.5]) == 0, 
                                min(year.t)-1, max(year.t[!select.nonNA & year.t < 1985.5]))
        ...
        ## edit DJS 2017-07-06
        if(special.constant){
          select.nonNA.flat <- apply(u1.hivcrisisfree.tj.flat, 1, function(x) sum(is.na(x)) == 0)
          ntrajs.NA.flat <- apply(u1.hivcrisisfree.tj.flat, 1, function(x) sum(is.na(x)))
          select.trajs.NA.flat <- apply(u1.hivcrisisfree.tj.flat, 2, function(x) sum(is.na(x)) > 0)
          if (sum(!select.nonNA.flat) > 0) {
            cat(paste0("Warning: ", mcmc.meta$data.all$name.c, " ", mcmc.meta$data.all$iso.c, "flat - NA obtained for the years: ", 
                       paste(year.t[!select.nonNA.flat], collapse = ", "), ".\n"))
            if (length(year.t[year.t >= 1985.5 & ntrajs.NA.flat > 0]) > 0) {
              cat(paste0("Note: ", mcmc.meta$data.all$name.c, " ", mcmc.meta$data.all$iso.c, " - The years (only 1990.5 onwards shown) ", 
                         paste(year.t[year.t >= 1985.5 & ntrajs.NA.flat > 0], collapse = ", "),
                         " have ",
                         paste(ntrajs.NA.flat[year.t >= 1985.5 & ntrajs.NA.flat > 0], collapse = ", "),
                         " trajectory(ies) with NA values respectively.\n"))
              # replace NA with min or max value
              for (t.select in (1:nrow(u1.hivcrisisfree.tj.flat))[!select.nonNA.flat]) {
                u1.hivcrisisfree.tj.flat[t.select, select.trajs.NA.flat] <- 
                  GetEstimateViaMLT(x.obs = u5.hivcrisisfree.tj[t.select, select.trajs.NA.flat],
                                    result = "IMR", allow.na = FALSE,
                                    lifetable = sub("MLT ", "", mcmc.meta$data.all$imrmethod.c),
                                    lifetable.file = lifetable.file)
              }
            ...
            year.cutoff.flat <- ifelse(length(year.t[!select.nonNA.flat & year.t < 1985.5]) == 0, 
                                  min(year.t)-1, max(year.t[!select.nonNA.flat & year.t < 1985.5]))
            year.t.final.flat <- year.t[year.t > year.cutoff.flat] 
            u1.hivcrisisfree.tj.flat <- u1.hivcrisisfree.tj.flat[is.element(year.t, year.t.final.flat), ]
          } else {
            year.t.final <- year.t
          } # end select.nonNA loop
        } # if special constant
      ...
      if (!is.element(mcmc.meta$data.all$imrmethod.c, c("B3", "Nonstandard"))) {
        ...
        #edit DJS 2017-06-07
        if(special.constant){
          u1.hivcrisisfree.tj.flat[u1.hivcrisisfree.tj.flat > 1000] <- 1000
          save(u1.hivcrisisfree.tj.flat, file = file.path(output.dir, paste0("u1.hivcrisisfree.tj.flat_w", w, ".rda")))  
        }
        #end edit DJS
      } else {
        u1.hivcrisisfree.tj[u1.hivcrisisfree.tj > 1000] <- 1000
      ...
      # edit DJS 2017-06-07
      if(special.constant){
        res.flat <- CalculateQuantities(u5temp.tj = u1.hivcrisisfree.tj.flat,
                                   iso = mcmc.meta$data.all$iso.c,
                                   indicator.type = indicator.type,
                                   # note: no further adjustments needed if relative adjustment is used, estimates are final 
                                   hiv = ifelse(!is.element(mcmc.meta$data.all$imrmethod.c, c("B3", "Nonstandard")), 
        ...
        if (w == 1) {
          u1.ctj.flat <- u1.ctj
          u1.ctj.flat[1, is.element(year.t, year.t.final), ] <- res.flat$u5.tj
        } else {
          eval(parse(text = paste0("u1new", w-1, ".ctj.flat <- u1new", w-1, ".ctj")))
          eval(parse(text = paste0("u1new", w-1, ".ctj.flat[1, is.element(year.t, year.t.final), ] <- res.flat$u5.tj")))
        }
        resall.cqt.Lw.flat <- resall.cqt.Lw
        resall.cqt.Lw.flat[[paste(weight.alpha)]][1, , is.element(year.t, year.t.final)] <- res.flat$resall.qt
        res.cqt.Lw.flat <-  res.cqt.Lw
        res.cqt.Lw.flat[[paste(weight.alpha)]][1, , is.element(year.t, year.t.final)] <- res.flat$res.qt
        resmean.ct.Lw.flat <- resmean.ct.Lw
        resmean.ct.Lw.flat[[paste(weight.alpha)]][1, is.element(year.t, year.t.final)] <- res.flat$resmean.t
        resARR.cq.Lwy.flat <- resARR.cq.Lwy
        for (y in 1:4) {
          resARR.cq.Lwy.flat[[paste(weight.alpha)]][[y]][1, ] <- unlist(res.flat$resARR.q.Ly[[y]])
        } 
      }
      # end edit DJS 
      ...
      # edit DJS 2017-06-07
      if(special.constant){
        res.hivremoved.cqt.Lw.flat <- res.crisisremoved.cqt.Lw.flat <- 
          res.crisisandhivremoved.cqt.Lw.flat <- res.logscale.hivremoved.cqt.Lw.flat <- res.cqt.Lw.flat
        
        res.hivremoved.cqt.Lw.flat[[paste(weight.alpha)]] <- res.crisisremoved.cqt.Lw.flat[[paste(weight.alpha)]] <- 
          res.crisisandhivremoved.cqt.Lw.flat[[paste(weight.alpha)]] <- 
          res.logscale.hivremoved.cqt.Lw.flat[[paste(weight.alpha)]] <- 
          res.cqt.Lw.flat[[paste(weight.alpha)]] 
        
        u.median.t.flat <- res.cqt.Lw.flat[[paste0(weight.alpha)]][1, percentiles == 0.5, 
                                                                   is.element(year.t, year.t.final)]
        if (mcmc.meta$data.all$crisisadj.c) {
          # get crisis-free results that include only HIV adjustments
          propadj.t <- GetCrisisAdjustedEstimates(u.t = u.median.t.flat, year.t = year.t.final, 
                                                  iso = mcmc.meta$data.all$iso.c,
                                                  operation = "-",
                                                  adj.file = adj.file)$propadj.t
          res.crisisandhivremoved.cqt.Lw.flat[[paste0(weight.alpha)]][1, , is.element(year.t, year.t.final)] <- 
            res.crisisremoved.cqt.Lw.flat[[paste0(weight.alpha)]][1, , is.element(year.t, year.t.final)] <- 
            t(apply(res.cqt.Lw.flat[[paste0(weight.alpha)]][1, , is.element(year.t, year.t.final)], 
                    1, "*", propadj.t))
          
        }
      }
      ## end edit DJS 2017-06-07
      ...
      # edit DJS 2017-06-07
      if(special.constant){
        res.logscale.hivremoved.cqt.Lw.flat[[paste0(weight.alpha)]] <- 
          log(res.hivremoved.cqt.Lw.flat[[paste0(weight.alpha)]])
        
        ## replace percentile=0.5 in output files with percentile=0.5 from flat out for special constant countries 
        res.hivremoved.cqt.Lw[[paste(weight.alpha)]][,percentiles==0.5,] <- res.hivremoved.cqt.Lw.flat[[paste(weight.alpha)]][,percentiles==0.5,]
        res.crisisremoved.cqt.Lw[[paste(weight.alpha)]][,percentiles==0.5,] <- res.crisisremoved.cqt.Lw.flat[[paste(weight.alpha)]][,percentiles==0.5,]
        res.crisisandhivremoved.cqt.Lw[[paste(weight.alpha)]][,percentiles==0.5,] <- res.crisisandhivremoved.cqt.Lw.flat[[paste(weight.alpha)]][,percentiles==0.5,]
        res.logscale.hivremoved.cqt.Lw[[paste(weight.alpha)]][,percentiles==0.5,] <- res.logscale.hivremoved.cqt.Lw.flat[[paste(weight.alpha)]][,percentiles==0.5,]
        res.cqt.Lw[[paste(weight.alpha)]][,percentiles==0.5,] <- res.cqt.Lw.flat[[paste(weight.alpha)]][,percentiles==0.5,]
      }
      # end edit DJS
