...
ConstructOutput <- function( # Construct output for MCMC run
  ...
  for (c in 1:C) {
    ...
    #  add by YS, to keep the rate constant (control alpha)
    if (!is.null(mcmc.meta$settings$special.constant.list)){
      ...
      k.constant.start = min(k.constant)
      k.constant.end = max(k.constant)
    
      for (k in c(k.constant,k.constant.end+1)){
        alpha.cpj[c, k, ] <- alpha.cpj[c, k.constant.start-1, ] 
      }
    }
