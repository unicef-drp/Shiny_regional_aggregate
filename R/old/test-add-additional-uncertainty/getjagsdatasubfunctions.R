...
GetJAGSDataForSplines <- function( # Get JAGS data list for splines
  ...
  year.t <- seq(floor(min(ifelse(is.element("RUS", data$iso.c), 1971, 1991), 
  ...
  for (c in 1:data$C) {
    ...
    year.min <- floor(min(ifelse(data$iso.c[c] == "RUS", 1971, 1991), 
    ...
    if (length(y.i) > 2) {
      if (indicator.type == "U5MR") {
        if (!includeIncompleteVRAny) {
          mod <- lme(fixed = y.i ~ -1 + BG.im, random = list(one = pdIdent(~ Z.iq - 1)))
        } else {
  ...
  jags.data.splines <- list(
    ...
    q.c = q.c,
    qplus1.c = q.c + 1,
    useWorldsigmau.c = c(ifelse(data$useglobalsmoothing.c, 1, 0), NA) # change JR, 20140502 # change JR, 20140516
  )
  if (indicator.type == "U5MR")
...
GetJAGSDataForNonVRObservations <- function(
  ...
  year.ci <- se.ynonvr.ci <- series.ci <- 
    se.ynonvradditional.ci <- # adhoc change JR, 20150526: added additional SE
    ...
    if (data$nnonvr.c[c] > 0) {
      ...
      se.ynonvr.ci[c, 1:data$nnonvr.c[c]] <- ifelse(!is.na(unlist(data$se.Lcs.j[[c]])),
                                                    ...
                                                    ifelse(paste(unlist(data$sourcetype.Lcs.j[[c]]), 
                                                           ...
                                                           se.censusindirect.missing, 
                                                           se.othernonvr.missing))
      # adhoc change JR, 20150526: added additional SE for MM-adjusted data
      se.ynonvradditional.ci[c, 1:data$nnonvr.c[c]] <- ifelse(data$hiv.c[c] & 
                                                           unlist(data$year.Lcs.j[[c]]) >= 1980,
                                                         0.1, 0)
  ...
  if (Cnonvr > 0) {
    ...
    if (!all(is.na(se.ynonvradditional.ci))) # adhoc change JR, 20150526. don't add se.ynonvradditional.ci if all NA otherwise BUGS error
      jags.data.nonvr <- c(jags.data.nonvr, list(se.ynonvradditional.ci = se.ynonvradditional.ci))
    if (run.type == "global")
...
GetJAGSDataForVRObservations <- function(
  ...
  # adhoc change JR, 20150526: added additional SE
  se.yvradditional.ci <- matrix(NA, data$C+1, data$nmax)
    ...
    if (data$nvr.c[c] > 0) {
      ...
      if (input.vr.se) {
        ...
        se.yvr.ci[c, select.vr.i] <- ifelse(!is.na(unlist(data$sevr.Lc.j[[c]])),
                                            ...
                                            se.vr.missing)
        # adhoc change JR, 20150526: added additional SE
        # EST add SE of 0.1 up to 1991!
        se.yvradditional.ci[c, select.vr.i] <- ifelse(data$iso.c[c] %in% 
                                                        c("DZA", "BGR", "CZE", "ESP", "GRC", "LVA", 
                                                          "ROU", "RUS", "SVK"), 0.1,
                                                      ifelse(data$iso.c[c] %in% c("BLR", "HUN", "LTU"),
                                                             0.05, 0))
  ...
  if (Cvr > 0) {
    ...
    if (input.vr.se & !all(is.na(se.yvradditional.ci))) # adhoc change JR, 20150526. don't add se.yvradditional.ci if all NA otherwise BUGS error
      jags.data.vr <- c(jags.data.vr, list(se.yvradditional.ci = se.yvradditional.ci))
    if (hasVRBias) # change JR, 20140429
