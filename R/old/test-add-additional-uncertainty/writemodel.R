...
WriteModel <- function( # Write JAGS model out as a .txt file
  ...
  for (d in 1:Cnonvr) {
    for (j in 1:nnonvr.c[getc.nonvr.d[d]]) {
      ...
      var.y.ci[getc.nonvr.d[d], geti.nonvr.cj[getc.nonvr.d[d], j]] <- (
        ...
        # adhoc change JR, 20150526: added SE for countries with data adjustments 
        # (countries with poor VR data, Eastern European countries with adjusted VR, 
        # countries with MM adjustments)
        ifelse(!all(is.na(mcmc.meta$jags.data$se.ynonvradditional.ci)), " 
        + pow(se.ynonvradditional.ci[getc.nonvr.d[d], geti.nonvr.cj[getc.nonvr.d[d], j]], 2)", ""), 
  ...
  for (d in 1:Cvr) {
    for (j in 1:nvr.c[getc.vr.d[d]]) {
      logbias.ci[getc.vr.d[d], geti.vr.cj[getc.vr.d[d], j]] <- ", ifelse(is.null(mcmc.meta$jags.data$is.vrwithbias.ci), "0", "
        ...
        # countries with MM adjustments)
      "
      tau.y.ci[getc.vr.d[d], geti.vr.cj[getc.vr.d[d], j]] <- 
        pow(var.y.ci[getc.vr.d[d], geti.vr.cj[getc.vr.d[d], j]], -1)", 
      ifelse(input.vr.se, paste0("
      var.y.ci[getc.vr.d[d], geti.vr.cj[getc.vr.d[d], j]] <- 
        pow(se.yvr.ci[getc.vr.d[d], geti.vr.cj[getc.vr.d[d], j]], 2)", 
      ifelse(!all(is.na(mcmc.meta$jags.data$se.yvradditional.ci)), " +
             pow(se.yvradditional.ci[getc.vr.d[d], geti.vr.cj[getc.vr.d[d], j]], 2)", "")), "
      var.y.ci[getc.vr.d[d], geti.vr.cj[getc.vr.d[d], j]] <- var.yvr.c[getc.vr.d[d]]"), "
    } # end j loop",
    ifelse(input.vr.se, "", "
    var.yvr.c[getc.vr.d[d]] <- pow(sigma.yvr.c[getc.vr.d[d]], 2)
  ...
  ifelse(input.vr.se, "", "
  # tau.yvr.c[C+1] <- 0 # dummy ############### adhoc change JR, 20150526
  var.yvr.c[C+1] <- 0 # dummy ############### adhoc change JR, 20150526
