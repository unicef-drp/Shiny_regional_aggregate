...
CleanDataFromCMEInfo <- function(# Reads in and clean data file from CME Info.
  ...
  data.cmeinfo <- read.csv(file = data.cmeinfo.file, header = T, stringsAsFactors = F)
  ...
  if (indicator.type == "IMR") {
    ...
    if (n.exclude > 0)
      cat(paste0(n.exclude, " observations with observation year earlier than the minimum U5MR observation year excluded.\n"))
  }
  #----------------------------------------------------------------------
  select.estimatezero <-  select.olddata <- select.sps <- select.excluded <- 
    select.countrynotselect <- select.hiv <- rep(F, nrow(data.cmeinfo))
  select.estimatezero <- data.cmeinfo$Estimates == 0 & data.cmeinfo$Inclusion == 1  
 if (any(select.estimatezero))
   cat(paste0(sum(select.estimatezero), " observations with Estimate = 0 and Inclusion = 1 excluded.\n"))
  if (any(select.estimatezero & data.cmeinfo$Inclusion == 1))
    cat(paste0("Warning: Of the above observations, ", sum(select.estimatezero & data.cmeinfo$Inclusion == 1), 
               " observations with Estimate = 0 and Inclusion = 1 were excluded.\n"))
  select.olddata <- !is.na(data.cmeinfo$Exclusion.Old.Data)
  if (any(select.olddata))
    cat(paste0(sum(select.olddata), " outdated observations (!is.na(Exclusion.Old.Data)) excluded.\n"))
  ...
  if (any(select.sps))
    cat(paste0(sum(select.sps), " observations from Sudan pre secession excluded.\n"))
  if (!includeExcluded) {
    select.excluded <- data.cmeinfo$Inclusion == 0
    if (any(select.excluded))
      cat(paste0(sum(select.excluded), " observations with Inclusion = 0 excluded.\n"))
  }
  if (!is.null(iso.select)) {
    ...
    if (!include.HIV.countries) {
      ...
      if (any(select.hiv))
        cat(paste0(sum(select.hiv), " observations for HIV countries excluded.\n"))
    }
  }
