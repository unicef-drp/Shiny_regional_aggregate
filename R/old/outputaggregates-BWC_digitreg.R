...
OutputAggregates <- function( # Calculate and output aggregated rates and numbers of deaths at the country,
  ...
  regiontypes.select = c("UNICEFProg", "UNICEFReport", "MDG", "SDGSimple", "WHO", "WB", "UNPD", "OIC",
                         ...
                         "USAID", "M49","GlobalStrategy"), ##<< Output regional aggregates for which region types? 
  ...
  # test
  if (test) {
    u5mrfinal.ctj <- u5mrfinal.ctj[, , 1:5]
    imrfinal.ctj <- imrfinal.ctj[, , 1:5]
    if(exists("nmrfinal.ctj")) nmrfinal.ctj <- nmrfinal.ctj[, , 1:5]
  }
...
CalculateWorldDeathsBWC <- function(
  ...
  if(nn.exists){
    res.world <- cbind(est.years.floor,
                       ...
                       roundoff(t(u5mr.qwt[, 1, ]), digits = 6),#lh replace ndigits with 6
                       roundoff(t(imr.qwt[, 1, ]), digits = 6),#lh replace ndigits with 6
                       roundoff(t(nmr.qwt[, 1, ]), digits = 6),#lh replace ndigits with 6
  ...
  if (dim(u5mr.wtj)[3] == 1) { # change JR, 26 Aug 2013
    u5mr.wtj <- roundoff(u5mr.wtj, digits =  6)#lh replace ndigits with 6
    imr.wtj <- roundoff(imr.wtj, digits =  6)#lh replace ndigits with 6
  }
...
GetRegionalResultsBWC <- function(
  ...
  CombineAndOutputRegionalResults(output.dir = output.dir,
                                  ...
                                  filename = filename,
                                  percentiles = percentiles,
                                  ndigits =  6)#lh replace ndigits with 6)
}
#----------------------------------------------------------------------
  ...
  for (r in 1:nregs) {
    if (filename %in% c("UNICEFProgRegion", "UNICEFReportRegion", "MDGRegion", "SDGRegion", "SDGSimpleRegion", "WBRegion", "UNPDRegion", "OICRegion", "M49Region", "Wealthall", "Wealthdata")) {
      ...
      # ifelse(is.na(match(which.no.rates,select.reg.og)), select.reg <- select.reg.og, select.reg <- select.reg.og[-match(which.no.rates,select.reg.og)])
                               ...
                               "FragileCountries2017", "WealthallGlobal", "WealthdataGlobal", "WorldBankReg2", "NewWorldBank", "USAIDCountries")) {
...
CombineAndOutputRegionalResults <- function(
      ...
      res.year <- rbind(res.year,
                        rbind(
                          cbind(est.years.floor[i],
                                ...
                                roundoff(t(u5mr.qrt[,,i]), digits =  6),#lh replace ndigits with 6
                                roundoff(t(imr.qrt[,,i]), digits =  6),#lh replace ndigits with 6
                                roundoff(t(nmr.qrt[,,i]), digits =  6),#lh replace ndigits with 6
  ...
  if (dim(u5mr.rtj)[3] == 1) { # change JR, 26 Aug 2013
    u5mr.rtj <- roundoff(u5mr.rtj, digits =  6)#lh replace ndigits with 6
    imr.rtj <- roundoff(imr.rtj, digits =  6)#lh replace ndigits with 6
  }
