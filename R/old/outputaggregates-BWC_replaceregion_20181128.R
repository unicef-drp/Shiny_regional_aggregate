  ...
  for (r in 1:nregs) {
    if (filename %in% c("UNICEFProgRegion", "UNICEFReportRegion", "MDGRegion", "SDGRegion", "SDGSimpleRegion", "WBRegion", "UNPDRegion", "OICRegion", "M49Region", "Wealthall", "Wealthdata")) {
      ...
      # ifelse(is.na(match(which.no.rates,select.reg.og)), select.reg <- select.reg.og, select.reg <- select.reg.og[-match(which.no.rates,select.reg.og)])
                               ...
                               "FragileCountries2017", "FragileCountries2018", "WealthallGlobal", "WealthdataGlobal", "WorldBankReg2", "NewWorldBank", "USAIDCountries")) {
    ...
    for(k in 1:dim(u5mr.temp.rt)[1]){
      ...
      for(yrk in min(years.k):max(years.k)){
        deathu5.temp.rt[k,match(yrk,years)] <- sum(roundoff(dx.mat.k[floor(years.k.mat)==yrk], digits = 0), na.rm=T) # DJS edit 2018-07-27 to round deaths at country level before regional aggregate
        death0.temp.rt[k,match(yrk,years)] <- sum(roundoff(dx.mat.k[1:2,][floor(years.k.mat[1:2,])==yrk], digits = 0), na.rm=T)
        if(nn.exists) deathnn.temp.rt[k,match(yrk,years)] <- sum(roundoff(dx.nn.mat.k[floor(years.k.mat[1:2,])==yrk], digits = 0), na.rm=T)
      } # for loop for summing deaths
