...
ConstructOutput <- function( # Construct output for MCMC run
  ...
  if(!is.null(mcmc.meta$settings$special.constant.list)){ ## edit DJS 20170526
    res.cqt.Lw.adj <- resnew0.cqt.Lw.adj
    res.cqt.Lw.adj[,2,] <- res.cqt.Lw.flat[,2,]
    OutputResultsWide(res.cqt = res.cqt.Lw.adj,
  ...
  if (run.type == "country") {
    if(!is.null(mcmc.meta$settings$special.constant.list)){ ## edit DJS 20170526
    results <- OutputResultsLong(res.qt = res.cqt.Lw.adj[1, , ], 
  ...
  if(!is.null(mcmc.meta$settings$special.constant.list)){ ## edit DJS 20170526
    save(res.cqt.Lw.adj, file = file.path(output.dir, "res.cqt.Lw.adj.rda"))
  ...
  if(!is.null(mcmc.meta$settings$special.constant.list)){ ## edit DJS 20170526
    res.hivremoved.cqt.Lw.adj <- res.crisisremoved.cqt.Lw.adj <- res.crisisandhivremoved.cqt.Lw.adj <- res.logscale.hivremoved.cqt.Lw.adj <- res.cqt.Lw.adj
    for (w in 1:nweightsplus1) {
      weight.alpha <- weights.alpha.plusdefault[w]
      # undo crisis post-adjustment for crisisadj countries
      for (c in (1:mcmc.meta$data$C)[mcmc.meta$data$crisisadj.c]) {
        u.median.t <- res.cqt.Lw.adj[c, percentiles == 0.5, ]
        # get crisis-free results that include only HIV adjustments
        propadj.t <- GetCrisisAdjustedEstimates(u.t = u.median.t, year.t = year.t, 
                                                iso = mcmc.meta$data$iso.c[c],
                                                operation = "-",
                                                adj.file = adj.file)$propadj.t
        res.crisisandhivremoved.cqt.Lw.adj[c, , ] <- 
          res.crisisremoved.cqt.Lw.adj[c, , ] <- 
          t(apply(res.cqt.Lw.adj[c, , ], 1, "*", propadj.t))
      }
      # undo HIV post-adjustment for countries with high HIV prevalence  
      for (c in (1:mcmc.meta$data$C)[mcmc.meta$data$hiv.c]) {
        # change JR, 20150602: fixed bug: assume relative uncertainty in 
        ...
        propadjhiv.t <- GetHIVAdjustedEstimates(u.t = u.median.t, year.t = year.t, 
                                                ...
                                                operation = "-",
                                                hiv.file = hiv.file)$propadjhiv.t
        res.hivremoved.cqt.Lw.adj[c, , ] <- 
          t(apply(res.cqt.Lw.adj[c, , ], 1, "*", propadjhiv.t))
        # get HIV and crisis-free results
        u.median2.t <- res.crisisremoved.cqt.Lw.adj[c, percentiles == 0.5, ]
        propadjhiv2.t <- GetHIVAdjustedEstimates(u.t = u.median2.t, year.t = year.t, 
                                                 iso = mcmc.meta$data$iso.c[c],
                                                 operation = "-",
                                                 hiv.file = hiv.file)$propadjhiv.t
        res.crisisandhivremoved.cqt.Lw.adj[c, , ] <-
          t(apply(res.crisisremoved.cqt.Lw.adj[c, , ], 1, "*", propadjhiv2.t))
      }
      res.logscale.hivremoved.cqt.Lw.adj <- 
        log(res.hivremoved.cqt.Lw.adj)
    }
    save(res.hivremoved.cqt.Lw.adj, 
         file = file.path(output.dir, paste0("res.hivremoved.cqt.Lw.adj", name.append)))
    save(res.crisisremoved.cqt.Lw.adj, 
         file = file.path(output.dir, paste0("res.crisisremoved.cqt.Lw.adj", name.append)))
    save(res.crisisandhivremoved.cqt.Lw.adj, 
         file = file.path(output.dir, paste0("res.crisisandhivremoved.cqt.Lw.adj", name.append)))
    save(res.logscale.hivremoved.cqt.Lw.adj, 
         file = file.path(output.dir, paste0("res.logscale.hivremoved.cqt.Lw.adj", name.append)))
    OutputResultsWide(res.cqt = res.hivremoved.cqt.Lw.adj,
                      name.c = mcmc.meta$data$name.c,
                      iso.c = mcmc.meta$data$iso.c,
                      year.t = year.t,
                      indicator.type = indicator.type,
                      ...
                      output.dir = output.dir,
                      file.name = "Results (HIV-free)")
    OutputResultsWide(res.cqt = res.crisisremoved.cqt.Lw.adj,
                      name.c = mcmc.meta$data$name.c,
                      iso.c = mcmc.meta$data$iso.c,
                      year.t = year.t,
                      indicator.type = indicator.type,
                      ...
                      output.dir = output.dir,
                      file.name = "Results (crisis-free)")
    OutputResultsWide(res.cqt = res.crisisandhivremoved.cqt.Lw.adj,
                      name.c = mcmc.meta$data$name.c,
                      iso.c = mcmc.meta$data$iso.c,
                      year.t = year.t,
                      indicator.type = indicator.type,
...
CalculateQuantities <- function( # Calculate relevant quantities such as quantiles of estimates and ARR.
  ...
  if (hiv) {
    propadjhiv.t <- GetHIVAdjustedEstimates(u.t = u.median.t, 
                                            ...
                                            hiv.file = hiv.file)$propadjhiv.t
    u5temp2.tj <- apply(u5temp.tj, 2, "*", propadjhiv.t)
  } else {
