...
PlotDataAndEstimates <- function(# Plot data, estimated fits and uncertainty intervals.
  ...
  zoom.year.end = 2015, ##<< Last year of zoom range for zoom plot.
...
AddSurveyData <- function(# Add survey data and/or sampling errors to plot
  ...
  if (is.null(col.s)) {
    ...
    # col.palette <- c(brewer.pal(12, "Paired")[-11], brewer.pal(12, "Set3")[-c(2,12)], 
    #                  brewer.pal(8, "Set2"), brewer.pal(8, "Pastel2")[-6])
    # col.palette <- c("red", "green", "purple", "pink", "orange", "darkgreen")
    col.s <- rep(col.palette, ceiling(nsurveys/length(col.palette)))[1:nsurveys]
    
    # # assign color by sourcetype
    # type.s <- paste(sourcetype.s, method.s)
    # col.s <- ifelse(type.s == "Census Indirect", "darkgreen",
    #                 ifelse(type.s == "MICS Indirect", "green",
    #                        ifelse(type.s == "Other Indirect", "blue",
    #                               ifelse(type.s == "DHS Direct", "pink",
    #                                      ifelse(type.s == "Other DHS Direct", "purple",
    #                                             ifelse(type.s == "Others Direct", "red", "orange"))))))
  } 
  ...
  for (s in series) {
    ...
    if (nobs == 1) {
      ...
      if (plot.se) {
        ...
        if (plot.lines & !(all(included) & any(hasbias | isincomplete))) # change JR, 20140429
          lines(u ~ year, col = col.s[si], lwd = lwd, lty = ifelse(!plot.data.excl, 1, 2))  
        if (plot.points) 
          points(u ~ year, pch = ifelse(hasbias, 23, ifelse(isincomplete, 24, 22)), # change JR, 20140429
