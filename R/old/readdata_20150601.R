...
ReadData <- function( # Read in country information and child mortality data and create a list with the data
  ...
  data.readin <- read.csv(file = data.file, header = T, as.is = T, stringsAsFactors = F, strip.white = T)
  ...
  # change JR, 20150515
#   for (c in (1:C)[crisisadj.c]) { # only need to check for countries with crisis adjustments
#     crisis.years.range <- range(data.adj$year.adj[data.adj$countrycode.adj == iso.c[c] & data.adj$add.adj != 0])
#     # if any observation year is within range of crisis years, crisisadjfordata is TRUE
#     crisisadjfordata.c[c] <- any(data.readin$year.i[data.readin$countrycode.i == iso.c[c]] >= crisis.years.range[1] & 
#                                    data.readin$year.i[data.readin$countrycode.i == iso.c[c]] <= crisis.years.range[2])
#     # if (iso.c[c] == "LKA") # adhoc change JR, 20150511
#     #   crisisadjfordata.c[c] <- FALSE
#   }
