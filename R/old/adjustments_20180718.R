...
GetCrisisAdjustedEstimates <- function( # Get crisis-adjusted estimates by adding WHO estimates of crisis-related mortality
  ...
  if (length(years.crisis) > 1) { 
    ...
    for(i in 1:n.adj.traj){
      ifelse(i==1,crisis.t <- ifelse(as.character(roundoffmidpoint(year.t)) == as.character(roundoffmidpoint(years.crisis)),data.adj[data.adj$countrycode.adj == as.character(iso),i+3],0),crisis.t<-rbind(crisis.t,ifelse(as.character(roundoffmidpoint(year.t)) == as.character(roundoffmidpoint(years.crisis)),data.adj[data.adj$countrycode.adj == as.character(iso),i+3],0)))
    } # i loop
    if(n.adj.traj==1) crisis.t <- t(as.matrix(crisis.t))
  ...
  propadj.t <- NULL
  if (is.null(u.t)) 
    u.t <- rep(0, length(year.t)) # u.t should not be null for adjustment country-years but will need to edit this to make matrix of adjustments as opposed to single adjustment
  u.unadj.t <- u.t
  if (operation == "+") {
  ...
  ##value<< List with
  return(list(u.t = u.t, ##<< Vector of crisis-adjusted estimates; now matrix as of 2017-07-05
              propadj.t = propadj.t ##<< Vector of crisis adjustment factors; now matrix as of 2017-07-05
  ))
}
