...
ResolvePlotColors <- function(n, base.colors = cp_UNICEF_div) {
  ...
  base.colors <- base.colors[!is.na(base.colors) & nzchar(base.colors)]
  sparse.colors <- base.colors[seq(2L, length(base.colors), by = 2L)]
  palette <- c(sparse.colors, base.colors[!base.colors %in% sparse.colors])

  if (length(palette) >= n) return(palette[seq_len(n)])
  grDevices::colorRampPalette(palette)(n)
}

RecodePlotIndicators <- function(x, indicator.order = NULL, recode.values = new_varname_list) {
  input.levels <- if (is.factor(x)) levels(x) else NULL
  x <- as.character(x)
  recoded <- dplyr::recode(x, !!!recode.values)

  if (is.null(indicator.order)) {
    indicator.order <- if (!is.null(input.levels)) input.levels else x
  }

  recoded.order <- dplyr::recode(as.character(indicator.order), !!!recode.values)
...
get.world.map <- function() {
  ...
  # Reproject before dissolving so unions are computed in planar coordinates.
  crs_now <- sf::st_crs(sfw)
  if (is.na(crs_now)) {
    sfw <- sf::st_set_crs(sfw, 4326)
  } else if (!identical(crs_now$epsg, 4326)) {
    sfw <- sf::st_transform(sfw, 4326)
  }
