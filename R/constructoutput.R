...
ConstructOutput <- function( # Construct output for MCMC run
  ...
  for (c in 1:C) {
    ...
    if (!is.null(mcmc.meta$settings$special.constant.list)){
      ...
      k.constant=which(resproject$alphayears.k >= special.constant.list[[1]][1] & resproject$alphayears.k <= special.constant.list[[1]][2])
    ...
    res <- CalculateQuantities(u5temp.tj = u5temp.tj,
                               ...
                               estyear.min = ifelse(mcmc.meta$data$iso.c[c] == "RUS", 1970.5, 1985.5),
    ...
    if (is.validation) {
      resfull <- CalculateQuantities(u5temp.tj = u5fulltemp.tj,
                                     ...
                                     crisisadj = mcmc.meta$data$crisisadj.c[c], adj.file = adj.file,
                                     year.t = year.t, year.i = year.i,
                                     estyear.min = ifelse(mcmc.meta$data$iso.c[c] == "RUS", 1970.5, 1985.5), # change JR, 9 Jun
                                     year1 = year1, year2 = year2, year3 = year3, 
                                     year4 = year4.input,
                                     percentiles = percentiles)
...
CalculateQuantities <- function( # Calculate relevant quantities such as quantiles of estimates and ARR.
  ...
  estyear.min = 1985.5, ##<< Minimum year for which estimates are required.
  year1 = 1990.5, ##<< First year used for ARR calculation.
  year2 = 2000.5, ##<< Second year used for ARR calculation.
  year3 = 2005.5, ##<< Third year used for ARR calculation, usually last year of estimation for validation exercise.
  ...
  if (crisisadj) {
    propadj.t <- GetCrisisAdjustedEstimates(u.t = u.median2.t, 
                                            year.t = year.t, iso = iso,
                                            adj.file = adj.file
                                            #,n.adj.traj=ncol(u5temp2.tj) # defaults to n.adj.traj=1 to use single adjustment value for all trajectories
                                            )$propadj.t
    u5.tj <- apply(u5temp2.tj, 2, "*", propadj.t)
    # u5.tj <- u5temp2.tj*t(propadj.t) # edit DJS 2017-08-09 for crisis uncertainty
  } else {
    u5.tj <- u5temp2.tj
  }
  if (!is.null(alpha.pj)) {
...
OutputResultsLong <- function(# Output results in long format
  ...
  # edit DJS 2018-03-09 remove rounding for results file
  # results <- data.frame(Year = year.t.output, 
  #                       Estimate = roundoff(res.qt.for.output.final[2,  ], digits = 5), # change JR, 22 Aug 2013: round to roundoff
  #                       Lower.bound = roundoff(res.qt.for.output.final[1, ], digits = 5), # change JR, 22 Aug 2013: round to roundoff
  #                       Upper.bound = roundoff(res.qt.for.output.final[3, ], digits = 5)) # change JR, 22 Aug 2013: round to roundoff
  results <- data.frame(Year = year.t.output, 
                        Estimate = res.qt.for.output.final[2,  ], 
                        Lower.bound = res.qt.for.output.final[1, ], 
                        Upper.bound = res.qt.for.output.final[3, ])
