...
message("Read in and check the file.\n Adhoc countries are ", paste(setDT(country.info)[AdhocCountries=="Adhoc", sort(CountryName)], collapse = ", "))
...
# #                  regiontypes.select = c("M49"),
# #                  get.world.results = FALSE,
# #                  test = FALSE, ##test=TRUE will sample 5 trajectories instead of full 8000
# #                  replace.rates.reg=NULL,
# #                  replace.rates.cat=NULL
# #                  )
# 
...
#                  filename.NMR = "finalresults.jtc.Rda",
#                  year.target = year.lastestimatepublished, est.years = seq(1950.5,year.lastestimatepublished,1),
#                  test = T, ## TEST RUN will sample 5 trajectories instead of full 8000
#                  regiontypes.select = c("SDGSimple","UNICEFProg","UNPD","MDG","SDGRegion"),#,#, "SDGSimple", "WB", "WHO", "UNPD"),
#                  replace.rates.reg=NULL,#"M49Region",
#                  replace.rates.cat=NULL)#replace(country.info$M49Region1, country.info$M49Region1=="Americas", country.info$M49Region2[country.info$M49Region1=="Americas"]))
# 
# # # Projections
# # source(file.path("R/outputaggregates-BWC.R"))
...
# #                  results.NMR.file = file.path("output", runname.NMR, "Results_NMR_CF2019-08-19.csv"),
# #                  run.on.server = run.on.server,
# #                  year4 = year.lastestimatepublished,
# #                  get.world.results = FALSE,
# #                  output.dir = file.dir.median,
# #                  year.target = 2018.5, est.years = seq(1950.5,2018.5,1),
# #                  regiontypes.select = "UNICEFReport",
# #                  replace.rates.reg=NULL,
# #                  replace.rates.cat=NULL) ## add livebirths.file?
# 
# 
# 
# 
...
# date.med <- "2019-08-15" 
# date.ui <- "2019-08-15"
# date.final <- "2019-08-15"
#  
# file.dir.median <- file.path(paste("Aggregate results (median)", date.med)) ##<< File directory to save median estimates to
# file.dir.ui <- file.path(paste("Aggregate results (UIs)", date.ui)) ##<< File directory to save UIs to
# file.dir.output <- file.path(paste("Aggregate results (final)", date.final)) ##<< File directory to save final (combined) median estimates
...
#   estimates <- estimates[,match(names(estimates.median), names(estimates))]
# 
...
# # file.names <- list.files("C:/Users/dsharrow/Dropbox/UN IGME data/2019 Round Estimation/Code/Agggregate results (UIs) 2019-08-15")
...
#                  livebirths.file = "input/data_livebirths_male.csv",
#                  year.target = year.lastestimatepublished, est.years = seq(1950.5,year.lastestimatepublished,1),
#                  regiontypes.select = c("UNICEFReport", "SDGSimple", "WB", "WHO", "UNPD"),
#                  test=FALSE,
...
# # get country deaths and M49 replacement
# OutputAggregates(results.U5MR.file = NULL,
#                  results.IMR.file = NULL,
#                  results.NMR.file = NULL,
#                  runname.U5MR = "Sex_forDeathCalculation",
#                  runname.IMR = "Sex_forDeathCalculation",
#                  filename.U5MR = "u5mrfinal.m.ctj.rda",
#                  filename.IMR = "imrfinal.m.ctj.rda",
#                  population.file = "input/data_male_CMEpopulation_20150817.csv",
#                  run.on.server = run.on.server,
#                  year4 = year.lastestimatepublished,
#                  output.dir = file.dir.ui,
...
#                  runname.U5MR = "Sex_forDeathCalculation",
#                  runname.IMR = "Sex_forDeathCalculation",
#                  filename.U5MR = "u5mrfinal.f.ctj.rda",
#                  filename.IMR = "imrfinal.f.ctj.rda",
#                  population.file = "input/data_female_CMEpopulation_20150817.csv",
#                  run.on.server = run.on.server,
#                  year4 = year.lastestimatepublished,
#                  output.dir = file.dir.ui,
...
#   file.dir.median <- file.path(paste0("C:/Users/dsharrow/Dropbox/UN IGME Data/2019 Round Estimation/Code/Aggregate results (median) 2019-08-20 (",sex[i],")")) ##<< File directory to save median estimates to
#   file.dir.ui <- file.path(paste0("C:/Users/dsharrow/Dropbox/UN IGME Data/2019 Round Estimation/Code/Aggregate results (UIs) 2019-08-20 (",sex[i],")")) ##<< File directory to save UIs to
#   file.dir.output <- file.path(paste0("C:/Users/dsharrow/Dropbox/UN IGME Data/2019 Round Estimation/Code/Aggregate results (final) 2019-08-20 (",sex[i],")")) ##<< File directory to save final (combined) median estimates + UIs to
#   if(!file.exists(file.dir.output)) dir.create(file.dir.output, showWarnings = F)
#   
#   # # Country
#   # file.name.ui <- "Rates & Deaths_Country Summary.csv"
#   # file.name.med <- paste0("Rates & Deaths(ADJUSTED)_",sex[i],"_Country Summary.csv")
...
#   # Region & world
#   file.names <- list.files(file.dir.median)
#   file.names <- file.names[grepl("(ADJUSTED)", file.names)]
#   file.names <- file.names[!grepl("Country Summary", file.names)]
#   # get rid of any file names that don't need to be combined
#   file.names <- file.names[!grepl("UNPD", file.names)]
#   file.names <- file.names[!grepl("WHO", file.names)]
#   file.names <- file.names[!grepl("MDG", file.names)]
#   file.names <- file.names[!grepl("UNICEFProg", file.names)]
#   file.names <- file.names[!grepl("WB", file.names)]
#   file.names <- file.names[!grepl("SDGSimple", file.names)]
#   file.names <- file.names[!grepl("World", file.names)]
#   
#   for (file.name in file.names) {
#     # paste0("(ADJUSTED)_",sex[i])
#     file.name.ui <- sub(pattern=paste0("(ADJUSTED)_",sex[i]), replacement="", x=file.name, fixed=TRUE)
#     estimates.file <- file.path(file.dir.ui, file.name.ui)
#     estimates.median.file <- file.path(file.dir.median, file.name)
#     estimates <- read.csv(estimates.file, header = T, stringsAsFactors = F)
#     estimates.median <- read.csv(estimates.median.file, header = T, stringsAsFactors = F)
