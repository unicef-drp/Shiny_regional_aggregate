...
ChooseRegion <- function(
 ...
 if (regiontype == "UNICEFProgRegion") {
  ...
  r <- ChooseSDGRegion(region)
 ...
 } else if (regiontype == "FragileCountries2024OECD") {
   r <- ChooseFragile2024OECDRegion(region) # YL 2023.05
   ...
   stop("regiontype must be one of these: UNICEFProgRegion, UNICEFRegionSSA, UNICEFReportRegion, MDGRegion,
       ...
       SDGRegion, SDGSimpleRegion, SDGRCRegion, EAPRORegion, FCSCountries, FragileCountries2024OECD")
 }
 return(r)
}
#----------------------------------------------------------------------
...
AURECRegionAll <- c("Arab Maghreb Union (AMU)",  # 1
                    ...
                    "Economic Community of West African States (ECOWAS) 202402",  # 6
...
ChooseAURECRegion <- function(region) {
  ...
  AURECRegs6 <- c("Economic Community of West African States (ECOWAS) 202402")
...
SDGRCRegionAll <- c(
  ...
  "ESCAP_All countries", 
  # "ESCAP_ECO (ECO)", "ESCAP_Africa (AFR)", "ESCAP_Europe (Europe)", "ESCAP_Latin America (LAC)", "ESCAP_North America (NAM)", # 2024 removed
...
# Add Fragile 2024OECD -----------------------------------------------------------
Fragile2024OECDRegionAll <- c("Extreme to high fragility", "Medium ro low fragile", "Extreme fragility", "High fragility")

ChooseFragile2024OECDRegion <- function(region) {
  Fragile2024OECDRegs1 <- c("Extreme to high fragility", "Medium ro low fragile")
  Fragile2024OECDRegs2 <- c("Extreme fragility", "High fragility")
  
  r <- ifelse(is.element(region, Fragile2024OECDRegs1), 1,
              ifelse(is.element(region, Fragile2024OECDRegs2), 2, NA))
  return(r)
}
